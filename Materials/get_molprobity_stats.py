...
def main(path):
    dataset_dirs = [dataset_dir for system_dir in pathlib.Path(path).glob('*') for dataset_dir in (system_dir / 'processed_datasets').glob('*') if system_dir.is_dir() & dataset_dir.is_dir()]
